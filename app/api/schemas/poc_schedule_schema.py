...
class PointBase(BaseModel):
    ...
    locationCode: str = Field(max_length=5, title="Port Of Discharge", pattern =r"[A-Z]{2}[A-Z0-9]{3}")
...
class Transportation(BaseModel):
    model_config = ConfigDict(cache_strings='all')
    transportType: TRANSPORT_TYPE = Field(description='e.g:Vessel,Barge,Feeder,Truck,Rail,Truck / Rail,Intermodal')
    transportName: str | None = Field(title='Vehicle Type',max_length=40, description='e.g:VesselName',default=None)
    referenceType: str | None = Field(title='Reference Type', description='e.g:IMO',default=None)
    reference: int | str | None = Field(title='Reference Value', description='e.g:Vessel IMO Code',default=None)
...
class Schedule(BaseModel):
    ...
    pointFrom: str = Field(max_length=5, title="First Port Of Loading", pattern =r"[A-Z]{2}[A-Z0-9]{3}")
    pointTo: str = Field(max_length=5, title="Last Port Of Discharge", pattern =r"[A-Z]{2}[A-Z0-9]{3}")
    etd: datetime
    eta: datetime
