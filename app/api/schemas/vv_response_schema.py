...
class PortBase(BaseModel):
    ...
    portCode: str = Field(max_length=5, title="Port Of Discharge", pattern =r"[A-Z]{2}[A-Z0-9]{3}")
...
class VesselDetails(BaseModel):
    model_config = ConfigDict(cache_strings='all')
    vesselName: str | None = Field(title='Vessel Name', description='VesselName',max_length=100)
    imo: int | str | None = Field(max_length=7,title='Vessel IMO Code', description='Vessel IMO Code', pattern =r"[0-9]{7}")
...
class PortCalls(BaseModel):
    model_config = ConfigDict(cache_strings='all')
    seq: int|None = Field(ge=1,description='Sequence of Port Calls',default=None)
    bound: list[str]|str|None = Field(description='Direction Code',default=None)
    voyage: list[str]|str = Field(max_length=20)
    portEvent: PortEvent
    port: PortBase = Field(description="This could be point/port")
    estimateDate: datetime|None = Field(description="eventDate will be ignored for omitted port", default=None)
    actualDate: datetime|None = Field(description="eventDate will be ignored for omitted port", default=None)
...
class Product(BaseModel):
    ...
    vessel:VesselDetails = Field(title='Vessel Details', description='Vessel Name and IMO Code')
    ...
    calls: list[PortCalls] | None = Field(default=None, title='Number Of Port Call',description="The number of ports to be called")
