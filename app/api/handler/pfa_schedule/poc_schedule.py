...
async def get_voyage(point_from: str = Query(alias='pointFrom', default=..., max_length=5,regex=r"[A-Z]{2}[A-Z0-9]{3}",example='HKHKG',description='Search by either port or point of origin'),
                     ...
                     etd_start: date = Query(alias='etdStartDate', default=...,example=datetime.now().strftime("%Y-%m-%d"),description='YYYY-MM-DD'),
                     etd_end: date|None = Query(alias='etdEndDate', default=None,example=datetime.now() + timedelta(weeks=4),description='YYYY-MM-DD'),
                     ...
                     service_code: str | None = Query(alias='serviceCode',default=None,description='Search by either service code or service name',max_length=30),
                     page: int = Query(alias='offset',default=0,description='the offset for pagination start from 0 by default. each page contains 30 result'),
    ...
    product_id: UUID = uuid5(NAMESPACE_DNS,f'{scac}-p2p-api-{point_from}{point_to}{etd_start}{etd_end}{service_code}{page}')
    placeholder: dict = {"row_offset": (page*BATCH_SIZE),"max_rows":BATCH_SIZE, "pol": point_from, "pod": point_to, "etd_start": etd_start.strftime('%Y-%m-%d'),"etd_end": (etd_start + timedelta(weeks=4)).strftime('%Y-%m-%d')}
    ...
    if sql_result:
        poc_schedule = [
            poc_schedule_schema.Schedule.model_construct(scac=leg.get('SCAC'), pointFrom=(pol:=leg['POL_PORT_CODE']),
                                                         pointTo=(pod:=leg.get('POD_PORT_CODE') if leg.get('POD_PORT_CODE') else leg.get('SUB_POD_PORT_CODE')),
                    ...
                    poc_schedule_schema.Leg.model_construct(pointFrom={'locationCode': pol}, pointTo={'locationCode': pod}, etd=etd, eta=eta, transitTime=tt,
                                                            transportations={'transportType': 'Vessel','transportName': leg.get('VESSEL_NAME'),'referenceType': 'IMO','reference':  leg.get('VESSEL_IMO')},
                                                            services={'serviceCode': leg.get('SERVICE_CODE')} if leg.get('SERVICE_CODE') else None,
        ...
        sorted_schedules: list = sorted(poc_schedule, key=lambda tt: (tt['etd'], tt['transitTime']))
