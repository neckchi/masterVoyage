        ...
        while retries < self.max_retries:
            try:
                self.pool = oracledb.create_pool_async(
                    user=self.user, password=self.password, dsn=self.dsn, min=self.concurrency, max=self.concurrency, retry_count=5)
                logging.info(
                    f"Successfully created SeaSchedule Oracle DB pool")
                return self.pool
        ...
        while retries < self.max_retries:
            try:
                ...
                async with self.pool.acquire() as conn:
                    logging.info(
                        f'Acquire connection from DB pool - {session_id}')
                    yield conn
                    logging.info(
                        f'Release connection back to DB pool - {session_id}')
                    await self.pool.release(conn)
                    break
            except oracledb.Error as error_pool:
                logging.error(
                    f"Error creating connection from  DB pool: {error_pool}")
                retries += 1
                await asyncio.sleep(1)
